...
public class PopulateList extends AsyncTask<Void, Void, Void> {
    ...
    private final AdapterViewAllFiles mAdapter;
     ...
     */
    public PopulateList(AdapterViewAllFiles adapter,
    ...
    private void populateListView() {
        ...
        if (pdfFiles == null)
            ...
            List<FilePDF> pdfFilesWithEncryptionStatus = getPdfFilesWithEncryptionStatus(pdfFiles);
     ...
     * @return new list of {@link FilePDF} with encrypted status set
     */
    @WorkerThread
    private List<FilePDF> getPdfFilesWithEncryptionStatus(@NonNull List<File> files) {
        List<FilePDF> pdfFiles = new ArrayList<>(files.size());
        for (File file : files) {
            pdfFiles.add(new FilePDF(file, mAdapter.getPDFUtils().isPDFEncrypted(file.getPath())));
        }
        return pdfFiles;
    }
}