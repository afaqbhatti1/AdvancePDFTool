...
import pdf.pdftool.interfaces.DataSetChanged;
import pdf.pdftool.interfaces.EmptyStateChangeListener;
import pdf.pdftool.interfaces.ItemSelectedListener;
import pdf.pdftool.model.FilePDF;
import pdf.pdftool.util.DialogUtils;
import pdf.pdftool.util.DirectoryUtils;
import pdf.pdftool.util.FileSortUtils;
import pdf.pdftool.util.FileUtils;
import pdf.pdftool.util.PDFEncryptionUtility;
import pdf.pdftool.util.PDFRotationUtils;
import pdf.pdftool.util.PDFUtils;
import pdf.pdftool.util.PopulateList;
import pdf.pdftool.util.StringUtils;
import pdf.pdftool.util.WatermarkUtils;

import static pdf.pdftool.util.Constants.SORTING_INDEX;
import static pdf.pdftool.util.FileUtils.getFormattedDate;

public class AdapterViewAllFiles extends RecyclerView.Adapter<AdapterViewAllFiles.ViewFilesHolder>
    ...
    private final HelperDatabase mHelperDatabase;
     ...
     * @param feedItems                list containing {@link FilePDF}
     ...
     */
    public AdapterViewAllFiles(Activity activity,
                               List<FilePDF> feedItems,
        ...
        mHelperDatabase = new HelperDatabase(mActivity);
    ...
    public void onBindViewHolder(@NonNull ViewFilesHolder holder, final int pos) {
        ...
        final FilePDF pdfFile = mFileList.get(position);
    ...
    private void performOperation(int index, int position, File file) {
        switch (index) {
            ...
            case 6://Password Set
                mPDFEncryptionUtils.setPassword(file.getPath(), AdapterViewAllFiles.this);
                break;

            case 7://Password Remove
                mPDFEncryptionUtils.removePassword(file.getPath(), AdapterViewAllFiles.this);
                break;

            case 8://Rotate Pages
                mPDFRotationUtils.rotatePages(file.getPath(), AdapterViewAllFiles.this);
                break;

            case 9: // Add Watermark
                mWatermarkUtils.setWatermark(file.getPath(), AdapterViewAllFiles.this);
                break;
        }
    }
     ...
     * Sets pdf files
     *
     * @param pdfFiles list containing {@link FilePDF}
     */
    public void setData(List<FilePDF> pdfFiles) {
        mFileList = pdfFiles;
        notifyDataSetChanged();
    }
    ...
    private void deleteFiles(ArrayList<Integer> files ) {
        ...
        int messageAlert;
        int  messageSnackbar = R.string.snackbar_files_deleted;
        if (files.size() > 1) {
                                    ...
                                    public void onDismissed(Snackbar snackbar, int event) {
                                        if (undoClicked.get() == 0) {
                                            for (String path : filePath) {
                                                ...
                                                mHelperDatabase.insertRecord(fdelete.getAbsolutePath(),
    ...
    private void renameFile(int position, String newName) {
        FilePDF pdfFile = mFileList.get(position);
        ...
        if (oldfile.renameTo(newfile)) {
            ...
            notifyDataSetChanged();
            mHelperDatabase.insertRecord(newfilename, mActivity.getString(R.string.renamed));
        } else
